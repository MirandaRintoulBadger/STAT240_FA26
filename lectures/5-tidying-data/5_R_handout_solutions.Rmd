...
```{r}
starwars %>% 
  relocate(birth_year, species) %>%
  slice_max(birth_year)
```
...
```{r}
starwars %>%
  filter(eye_color == "black" & height > 200) %>%
  relocate(homeworld)
...
mean(female_chars$height, na.rm = T)
mean(male_chars$height, na.rm = T)
mean(herm_chars$height, na.rm = T)
mean(none_chars$height, na.rm = T)
...
```{r}
penguins %>%
  mutate(radius = bill_depth_mm / 2) %>%
  mutate(bill_vol = case_when(
    species == "Adelie" ~ (pi * radius^2 * bill_length_mm)/2,
    .default = (pi * radius^2 * bill_length_mm)/2.5
  )) %>%
...
```{r}
drinks %>%
  left_join(coupons, by = join_by(Cafe == Location)) %>%
  mutate(Discount_Price = case_when(
    !is.na(Discount) ~ Price * (1 - Discount),
    .default = Price
  )) %>%
  relocate(Discount_Price)
```
